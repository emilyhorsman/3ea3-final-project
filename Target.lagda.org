...
... | nothing =
        Crashed ⟨ pointer c , modify-registers label suc (registers c) ⟩
... | just ip =
...
pf-R₃-incr
  ...
  → (program : Program n)
  → (label : RegisterLabel)
  → (pf : inc (pointer c) ≡ nothing)
  → lookup label (registers (conf (small-step (INCR label) program c)))
      ≡ suc (lookup label (registers c))
pf-R₃-incr n c program label pf =
  begin
    lookup label (registers (conf (small-step (INCR label) program c)))
  ≡⟨ {!!} ⟩
    suc (lookup label (registers c))
...
pf-R₁-incr (suc n) c program nonempty antecedent label =
  ...
  ∎
  where
    -- Agda has enough information to know this is true!
